...
## §5.5. 与匿名化共享数据的关系（5/9 W19 后续建议 3 / P2）⭐

### Year 2+ 候选评估（不在 v1.1 实施）

如未来需要跨客户匿名化共享 mapped 数据：

```
Option A: _anonymized/ 目录 + 单独 .gitignore 白名单
  - 适合：少量脱敏数据 / 顾问跨客户复用
  - 限制：不适合 SQL 复杂查询
  - 触发：客户战役 ≥ 3 / mapped 数据 ≥ 1 万行

Option B: SQLite 独立表（_anonymized.db）
  - 适合：跨客户匿名化数据库化
  - 优势：FK 强保证 + 查询性能 + 事务
  - 触发：mapped 数据 ≥ 10 万行 + 客户战役 ≥ 5
```

### 强制约束（不可绕过）

```
❌ 不得把客户 raw/mapped 原始数据直接数据库化共享
❌ 不得跳过脱敏直接入 SQLite
❌ 不得无客户 sign off 的跨客户复用

✅ 必须先建立：
  1. 脱敏字段白名单（哪些字段可入匿名库）
     - 允许：标准 SKU 引用 / 品类骨架引用 / 匹配规则
     - 禁止：客户名 / 客户内部码 / 销售金额绝对值 / 毛利绝对值
  2. 客户脱敏规则（如客户名 → client_id / 销售额 → 区间值）
  3. 复用边界（哪些场景允许跨客户调用）
  4. 6 哥 + 启明 sign off
```
...
当 vault 同时满足：
  - 客户战役 ≥ 3 个完整收尾
  - mapped 数据累计 ≥ 1 万行
  - 跨客户复用需求明确（如品类对比 / 标杆诊断）
  - 6 哥 + 启明 sign off

→ 启动 _anonymized/ 或 SQLite 独立表评估
→ 评估通过后 / 单独立项设计
```
