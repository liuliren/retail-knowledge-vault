...
- RAW_CAT_TO_L3     : 通用"报表类别名→标准 L3"精确映射(跨店共用基座)
...
# 本体版本单点声明(版本口径只此一处;正文注释一律称"标准本体",防再次半迁移漂移)
ONTOLOGY_VERSION = "V6.0"  # 68 L3 / 385 L4 · SSOT · DEC-20260702-01

import sys, csv, re, os
from collections import defaultdict

# ── 品名关键词 → 标准本体 L3(命中即归类·优先级从上到下;具体在前,泛在后)──────────
L3_KEYWORDS = [
  # 糖巧果冻
...
# 复杂类(需 L4 决策组分层) — 品牌密度/价格带分裂型(标准本体命名)
COMPLEX_L3 = {"巧克力","白酒","啤酒","葡萄酒","低度酒/预调酒","黄酒米酒",
              "麦片谷物","奶粉冲调","咖啡冲饮","传统冲饮","豆奶冲饮",
              "果干类","蜜饯果脯类","凉果类","散称组合类"}

# ── 通用: 报表类别名(raw_category_name)→ 标准本体 L3 精确映射(跨店共用基座)──────────
...
# 源类目同义归一(类别名含子串时·标准本体命名)— 主要服务"有类目列但类别名非标"的其他店
...
# ── 二级规则: 泛源类目 + 品名关键词 → 标准 L3(scoped·只对登记的 raw_cat 生效, 零污染其他类)──
...
# 玩具六分(按购买动机+场景, 非传统品类) / 文具三分。落独立 l3_sub 字段, **不污染标准本体主 L3**;
# category_l3 仍取标准本体锚(见 LEAF_TO_STD_L3), 单本体不变。无法归类 → 基础玩具/基础文具 + needs_review。
LEAF_RULES = {
...
# leaf → 标准本体 L3 锚(保单本体; 空=该 leaf 无干净标准 L3 对应 → category_l3 留待确认)。
# ⚠ 玩具六分是"购买动机"切法, 与标准本体玩具潮玩 的"类型" L3(毛绒/积木/盲盒…)非同一维度,
#    一个动机跨多个标准 L3, 故玩具 leaf **不强配标准 L3**(category_l3=待确认), 细分只活在 l3_sub。
# 文具三分恰好对齐标准本体文具书写 L3(笔类/本册/修正工具), 故有干净锚。
LEAF_TO_STD_L3 = {
    "益智学习": "", "功能运动": "", "陪伴": "",
    "潮玩收藏": "", "节庆应季": "", "基础玩具": "",
...
def infer_leaf(name, src_cat):
    ...
    for leaf, kws in LEAF_RULES[src_cat]:
        if any(kw in nm for kw in kws):
            return leaf, LEAF_TO_STD_L3.get(leaf, ""), False
    d = LEAF_DEFAULT[src_cat]
    return d, LEAF_TO_STD_L3.get(d, ""), True
...
def infer_l3(name, src_cat, store_id=None):
    ...
    if cat_l3 == kw_hits[0]:
        ...
        if cat_method in ("源类目精确(标准本体)", "二级(源类目+品名)"):
