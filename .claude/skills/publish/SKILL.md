...
## Step 1.5 · 客户真名授权链硬检查（2026-07-07 P7-3红队测试P0修复·不可跳过）

> P7-3 红队压测发现：此前 `/publish` 只有 Step5 隐私检查里的软提示（可跳过），没有硬性拦截点防止未授权客户真名流到对外发布。本步骤是硬拦截门，检查不过 = 中止流水线，不得进 Step 2。

```bash
REG="retail-knowledge-vault/.claude/客户代号注册表_LOCAL.txt"
AUTH="David-Liu-Vault/80_个人资料_敏感/客户名称对外使用授权链清单.md"
ARTICLE="<文章完整路径>"
...
while IFS= read -r name; do
  [ -z "$name" ] && continue
  grep -q "$name" "$ARTICLE" && HIT="$HIT $name"
done <<< "$NAMES"
```

- 若 `$HIT` 为空 → 无客户真名命中，直接放行进 Step 2。
- 若 `$HIT` 非空 → 对每个命中的真名：
  1. 换算代号（`$REG` 里 `真名=代号`）。
  2. 去 `$AUTH` 查该代号的「对外用名授权」列 + 「授权范围」列，逐场景核对本篇内容属于「正面案例」还是「批评/诊断向」、渠道是否覆盖。
  3. **✅ 且场景覆盖** → 可保留真名，在发布记录里写一行"依据"（授权来源+日期）。
  4. **❌ / 部分不覆盖 / 查不到** → **硬中止**，输出：
     ```
     ⛔ GOV·客户真名授权链未过：命中「<真名>」，授权表状态=<❌/未覆盖/查不到>。
     本文不得携带真名对外发布。请先脱敏泛化（如"广州一家社区超市"）后重新起草，或六哥手动确认授权后重跑本步骤。
     ```
     **不进入 Step 2，流水线到此为止。**
...
| v0.4 | 2026-07-07 | P7-3红队测试P0修复：新增 Step1.5 客户真名授权链硬检查（原只有软提示，现命中未授权真名即硬中止流水线） |
