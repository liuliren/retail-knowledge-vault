...
> **真实数字使用判断句(2026-07-08 六哥签字·P1-GOV-DataUse-Judgment-001)**：分界线不是"能不能碰/写真实数据"，是"**这份产物完成后会不会跨客户复用、对外发布，或者混进 01-14 这类开放方法论层**"。会 → 脱敏/只留结构与聚合值；只留在该客户自己的项目文件夹（09_/16_）里、服务该客户自己的诊断 → 真实数字（如毛利率、GMROI 等**计算结果**）该有就有，这正是咨询交付本身。**红线仍是条码/进价/供应商名等原始行级明细**（见 C 层"只汇总"）——聚合计算值与原始行级明细不是同一回事，前者是诊断结论的一部分，后者才是本条要挡的对象。*(来源：99_归档补课精读工程中处理花厅坊 99_原始素材盘点任务时的现场判断讨论，六哥确认写入)*
