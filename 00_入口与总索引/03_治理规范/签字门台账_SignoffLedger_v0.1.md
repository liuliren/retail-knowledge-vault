...
| 2026-07-08 | 对外呈现层零售行话准则立项 | 一切对外呈现去IT化、用零售行话表达(例外=内部系统开发场景);列为核心任务持续推进;准则draft v0.1落库+行话对照表v0.1起步 | 六哥（口述核心裁决·录记忆区+治理规范双备份） | 培训材料三原则(同日)为其子集;三轮审议待走;/publish等skill挂钩待授权 |
| 2026-07-08 | 对外呈现层零售行话准则 v0.3 | draft→**stable**（三轮审议完成:盘点实证/魔鬼代言人184a2aa/行业steelman 4051180） | 六哥（行话治理系统收口任务书面签字） | 全库首个完整三轮审议生命周期标准件;核心原则=专业词分场景使用非去专业化;六机制定版;三试点确认(988已同步/底盘模板=三色标签SSOT唯一源/商品力无需回改);/critique --steelman保持candidate待跨场景验证(Case1训练包定稿审议·Case2 988销售页,角色按零售场景校正登记) |
| 2026-07-08 | 卡片生成模块审计·5问一次性裁决 | ①report-export candidate→active补签(追认好家源3店07-06/07实产PDF既成事实) ②渲染器边界=按输出形态分工(mdcard=内部快卡PNG,report-export=正式客户PDF+PNG) ③mdcard加`--spec-set`/`--client`/输出台账三能力,v1.1→v1.2升级 ④视觉SSOT暂不统一,记观察清单 ⑤流程矩阵查阅表补report-export列,v1.0→v1.1 | 六哥（AskUserQuestion五问·全部选推荐项） | 审计发现两套引擎两周未和解:mdcard承诺的两参数从未落地(全是手写单卡)、零输出台账；report-export已实产客户交付件却仍标candidate、mdcard裁决稿"未建成前不引用"已是假前提。详见 [[2026-07-08_卡片生成模块全面审计与升级方案_draft_v0.1]]；旧v0.2裁决稿标superseded指回本条 |
| 2026-07-08 | mdcard.py v0.5→v0.6 代码升级 | 加`--spec-set`(卡组批量)/`--client`(客户目录路由,查无则报错不乱建)/`output_ledger.csv`(每次渲染留痕);--spec/--out单卡模式不变 | 六哥（卡片生成模块审计裁决点3授权） | 合并前在部署路径(13_数据分析与工具脚本/图卡引擎_v0.1/)重新冒烟测试,抓出草稿脚本一个真实bug:VAULT_ROOT用`../../..`算多一层会跳出vault根到KnowledgeBase(草稿在Claude输出区测试时因目录深度偶然掩盖此错),已改回`../..`两级并复测通过(单卡回归+卡组+客户路由三项皆✓);遗留:测试产生的1个PNG(09_.../乐易购花厅坊店/合并验收_L0.png)被vault-guardian拦截无法用rm删除,需六哥手动清理或走apply-signoff.sh |
| 2026-07-09 05:44 | PostToolUse监听 | apply-signoff.sh | system | 已触发落账监听 |
| 2026-07-09 05:45 | D档删除 | /Users/davidliu/KnowledgeBase/retail-knowledge-vault/09_门店案例与项目复盘/乐易购花厅坊店/合并验收_L0.png | 六哥(apply-signoff) | 清单: /private/tmp/claude-501/-Users-davidliu/b5967b57-100e-458c-8096-b816d27466c7/scratchpad/cleanup_合并验收测试文件_2026-07-08.txt |
| 2026-07-09 05:45 | PostToolUse监听 | apply-signoff.sh | system | 已触发落账监听 |
| 2026-07-08 | 卡片生产系统v1.0收口·Phase2四件交付 | 建4份稳定化/质量化文档:CARD-SYSTEM-STATUS-AUDIT(active·五维校验通过,发现好家源PDF未归位待六哥定夺)/CARD-QUALITY-CHECKLIST(draft·冠"标准"字样未过三轮审议,仅供参考不强制卡关)/CARD-TEMPLATE-LIBRARY(draft·5类逻辑模板)/CARD-REPORT-PIPELINE-DESIGN(draft·只做设计不开发,建议留parking lot) | 六哥（Phase2任务书授权·最高原则"不合并/不重做视觉/不新增引擎/不为优化而优化"） | 全程未违反任务书四条禁令;Task2清理已走signoff（见上两行）;四文档落 Claude输出区,待六哥过目定去留 |
| 2026-07-08 | C档移动·好家源正式PDF归位 | 12个文件(html/pdf/png×4套)从`Claude输出区/`移入既有客户项目`09_门店案例与项目复盘/ZQ_破晓计划/`——经营底盘诊断报告→项目根记`09_`；三店商品力诊断报告→各自`05/06/07_XX首跑作业包/06_商品力诊断报告_正式版_v0.1.*`(接续该作业包既有01-05号命名序列) | 六哥（口令「需要」，回应Phase2审计"好家源PDF未归位"提问） | 移动前扫描零反链引用(grep确认无.md引用旧路径,移动不破链)；发现"好家源"实为ZQ_破晓计划项目下三店(顺景/富城/金沙咀)代号，非需新建的独立客户目录，纠正了上一条审计记录里"需新建好家源目录"的表述；文件均为gitignore覆盖的html/pdf/png，用mv非git mv；report-export的output_ledger.csv保留原路径记录不回改(视为不可变历史留痕，新路径仅记于此) |
